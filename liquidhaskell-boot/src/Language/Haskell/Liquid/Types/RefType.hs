...
eqRSort m t (RAllT _ t' _)
  = eqRSort m t t'
eqRSort m (RFun _ _ t1 t2) (RFun _ _ t1' t2')
...
nlzP ps t@(RVar _ _ )
 = (t, ps)
nlzP ps (RFun b i t1 t2)
 = (RFun b i t1' t2', ps ++ ps1 ++ ps2)
...
strengthenRefType_ f (RFun x1 i1 t1 t1') (RFun x2 i2 t2 t2') =
  -- YL: Evidence that we need a Monoid instance for RFInfo?
  if x2 /= F.dummySymbol
    then RFun x2 i1{permitTC = getFirst b} t t1''
    else RFun x1 i1{permitTC = getFirst b} t t2''
    where t  = strengthenRefType_ f t1 t2
...
freeTyVars (RFun _ _ t t') = freeTyVars t `L.union` freeTyVars t'
...
tyClasses (RFun _ _ t t') = tyClasses t ++ tyClasses t'
...
subsFree m s z (RFun x i t t')
  = RFun x i (subsFree m s z t) (subsFree m s z t')
...
mkRApp m s c ts rs r r'
  ...
  = RFun dummySymbol defRFInfo t1 t2
  | otherwise
...
instance SubsTy Symbol Symbol (BRType r) where
  ...
  subt su (RFun x i t1 t2)  = RFun x i (subt su t1) (subt su t2)
...
toType useRFInfo (RFun _ RFInfo{permitTC = permitTC} t@(RApp c _ _ _) t')
...
expandProductType x t
  | isTrivial'      = t
  | otherwise       = fromRTypeRep $ trep {ty_binds = xs', ty_info=is', ty_args = ts'}
     where
      isTrivial'    = ofType (varType x) == toRSort t
      τs            = map irrelevantMult $ fst $ splitFunTys $ snd $ splitForAllTyCoVars $ toType False t
      trep          = toRTypeRep t
      (xs',is',ts') = unzip3 $ concatMap mkProductTy $ zip4 τs (ty_binds trep) (ty_info trep) (ty_args trep)
...
mkProductTy :: forall r. Monoid r
            => (Type, Symbol, RFInfo, RType RTyCon RTyVar r)
            -> [(Symbol, RFInfo, RType RTyCon RTyVar r)]
mkProductTy (τ, x, i, t) = maybe [(x, i, t)] f (deepSplitProductType menv τ)
  where
    f    :: DataConAppContext -> [(Symbol, RFInfo, RType RTyCon RTyVar r)]
    f    DataConAppContext{..} = map ((dummySymbol, defRFInfo,) . ofType . fst) dcac_arg_tys
...
tyVarsPosition = go (Just True)
  where
    ...
    go p (RFun _ _ t1 t2) = go (flip' p) t1 <> go p t2
