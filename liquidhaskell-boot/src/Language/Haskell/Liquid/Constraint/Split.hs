...
splitC allowTC (SubC γ (RFun x1 i1 t1 t1') (RFun x2 i2 t2 t2'))
  =  do cs'      <- splitC allowTC  (SubC γ t2 t1)
        γ'       <- γ+= ("splitC allowTC", x2, t2)
        cs       <- bsplitC γ (RFun x1 i1 t1 t1')
                              (RFun x2 i2 t2 t2')
...
traceTy (RFun _ _ t t') = parens ("RFun " ++ parens (traceTy t) ++ parens (traceTy t'))
...
forallExprReftLookup :: CGEnv
                     -> F.Symbol
                     -> Maybe ([F.Symbol], [RFInfo], [SpecType], SpecType)
forallExprReftLookup γ sym = snap <$> F.lookupSEnv sym (syenv γ)
  where
    snap     = mapFourth4 ignoreOblig . (\((x,a,b),t)->(x,a,b,t)) . bkArrow . thd3 . bkUniv . lookup'
